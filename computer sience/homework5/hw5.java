...
public class hw5 {
    public static void main(String args[]){
        ...
        int current_phase = 0;//default phase
        Card[] P1_fight_area;
        Card[] P2_fight_area;
        ...
        Updator.download_Player_data(args[0], P1, P2, Arena);//get info from player_info.txt
        Updator.download_Fightground_data(args[1], Arena);//get info from playerground.txt
        if(Updator.save_is_downloaded()){
            game_loop = true;
        }
        else{
            game_loop = false;
        }// to check files are existance
        if(game_loop){
        ...
        while(game_loop && current_phase <= 10){
            ...
            for(int i = 0; i < 3; i++){
                if(Arena.get_P1_fight_area()[i] != null && Arena.get_P1_fight_area()[i].get_potion() != null){
                    Arena.get_P1_fight_area()[i].potion_over_a_turn();
                }
                if(Arena.get_P2_fight_area()[i] != null && Arena.get_P2_fight_area()[i].get_potion() != null){
                    Arena.get_P2_fight_area()[i].potion_over_a_turn();
                }
            }
